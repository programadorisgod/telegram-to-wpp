...
**Authorization precondition**: All scenarios in this specification implicitly require the sender to be authorized via `BOT_AUTHORIZED_CHAT_ID`. The authorization gate (`openspec/specs/auth/spec.md`) rejects unauthorized senders before any client management code is reached. No behavioral changes to existing scenarios — the gate is an earlier rejection layer.
